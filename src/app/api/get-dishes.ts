...
import { DishInterface } from "./get-dishes.types";
 ...
 */
export async function getDishes(restaurante: string, language: string, category: string) {
 ...
 */
export async function getDish(restaurante: string, language: string, category: string, dish: string) {
    return sql<DishInterface>`
        SELECT name, image, link, description, price
        FROM Dishes
