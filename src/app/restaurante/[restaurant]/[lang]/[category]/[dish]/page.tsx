...
export default async function DishPage({
    params: { dish, restaurant, lang, category },
}: DishPageProps) {
    const dishInfo = (await getByLink(restaurant, lang, category, dish)).rows;
    if (dishInfo.length === 0) {
        notFound();
    }
    const image =
        !dishInfo[0].image || dishInfo[0].image === ""
            ? defaultDishImg.src
            : dishInfo[0].image;
    return (
        <Background image={image}>
            <RestaurantHeader
                title={toTitle(dishInfo[0].name)}
                        ...
                        <Grid container rowSpacing={2}>
                            <Grid item xs={12} textAlign="center">
                                <Typography variant="body1">
                                    {dishInfo[0].description}
                                </Typography>
                            </Grid>
                            <Grid item xs={12} textAlign="center">
                                <Typography variant="h4">
                                    {formatPrice(dishInfo[0].price)}
                                </Typography>
                            </Grid>
                        </Grid>
                    </Grid>
