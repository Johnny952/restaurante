...
import { useMediaQuery, useTheme, Box, Paper, IconButton, Badge, Typography, Drawer, List, ListItem, ListItemButton, ListItemText, ListItemIcon } from '@mui/material';
...
const ResponsiveNavigation = ({
    ...
    const productCount = useFavStore((state) =>
        Object.values(state.products).reduce((sum, product) => sum + product.quantity, 0)
    );
    ...
    const btnColor = (disabled: boolean) => disabled ? grey[400] : purple[700];
    const voidFn = () => { };

    const mobileNavigation = (
        <>
            <NavBar elevation={3}>
                <NavButton onClick={() => !disableBack ? router.back() : null}>
                    <IconButton disabled={disableBack}>
                        <ArrowBackIosNewIcon sx={{ color: btnColor(disableBack) }} />
                    </IconButton>
                    <Typography variant="caption" sx={{ color: btnColor(disableBack) }}>Atrás</Typography>
                </NavButton>
                <NavButton onClick={handleListDrawerToggle}>
                    <IconButton disabled={disableCategories}>
                        <CategoryIcon sx={{ color: btnColor(disableCategories) }} />
                    </IconButton>
                    <Typography variant="caption" sx={{ color: btnColor(disableCategories) }}>Categorías</Typography>
                </NavButton>
                <Box width={60} />
                <NavButton onClick={handleLanguageToggle}>
                    <IconButton disabled={disableLanguage}>
                        <LanguageIcon sx={{ color: btnColor(disableLanguage) }} />
                    </IconButton>
                    <Typography variant="caption" sx={{ color: btnColor(disableLanguage) }}>Lenguajes</Typography>
                </NavButton>
            ...
            >
                <Box sx={{ width: '100%', backgroundColor: purple[50] }} role="presentation" onClick={handleMenuDrawerToggle}>
    ...
    const desktopNavigation = (
        <>
            <MenuButton setOpenFeed={setOpenFeed} setOpenCart={setOpenCart} handleLanguageToggle={handleLanguageToggle} handleListDrawerToggle={handleListDrawerToggle} disableCategories={disableCategories} disableLanguage={disableLanguage} />
    ...
    return (
        <Box sx={{ pb: 9 }}>
            ...
            />
            <ShoppingCart open={openCart} onClose={() => setOpenCart(false)} />
        </Box>
    );
};
